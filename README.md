...
- 这个和[4网页或md转word](#4网页或md转word)功能相辅相成。
- 对https://www.gov.cn/进行了清洗规则适配。
...
### 4. 变量管理
- 所有变量统一存储在 `st.session_state` 字典中
- 支持变量值的自动保存和恢复
...
##### 2.1 布尔类型条件判断
从st.session_state中获取条件名变量的值，判断是否为True。如果为True，则渲染if_body中的内容；如果为False，则渲染else_body中的内容。
```
{% if 条件名 %}
  条件成立时显示的内容
{% else %}
...
##### 2.2 表达式条件判断
从st.session_state中获取变量名的值，判断是否为"值"或"其他值"。如果为"值"，则渲染if_body中的内容；如果为"其他值"，则渲染elif_body中的内容；如果都不是，则渲染else_body中的内容。
```
{% if 变量名 = "值" %}
  条件成立时显示的内容
{% elif 变量名 = "其他值" %}
...
- **开关名**：控件的变量名，用于在会话状态中存储用户选择的值
- 渲染时会创建一个开关组件，用户打开表示条件成立，关闭表示条件不成立
- 默认状态为True
...
解决{}占位符问题，直接在word模板中使用{}进行批量文件的生成。
